...
p = NVProgram(dev, TinyELF(lib=open("~/tinygrad-metal/splitkv/pv_full64.cubin","rb").read(),
      name="pv_full64", target=dev.renderer.target, signature=(("v",0,dtypes.int32,()),)))
p(ob, *bufs, global_size=(3,24,2), local_size=(128,1,1), vals=(0,), wait=True)
o = outT.numpy()
print(f"[pv_f64] o[:2]={o[:2]} max={o.max():.4f} (expect ~64*sig(1)={64/(1+np.exp(-1)):.2f})", flush=True)
