...
pos = int(_os.getenv("REPRO_START", "900"))
t0 = time.perf_counter()
while pos < L - 4:
    dev.allocator._copyin(_TOK.uop.buf_uop.buffer._bufs["NV"], memoryview(_np.asarray([[1, 2, 3]], dtype=_np.int32).tobytes()).cast("B"))
    with Context(JIT=1):
        ret = probe_j(_TOK, v_sp.bind(pos), True)
        h = ret[0].contiguous().realize(); am = ret[1].contiguous().realize()
    dev.synchronize()
    print(f"[pos {pos}] ok ({time.perf_counter()-t0:.1f}s) am={am.numpy()[0]}", flush=True)
    pos += 8
