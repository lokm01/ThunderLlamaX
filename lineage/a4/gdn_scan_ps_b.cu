...
// gdn_scan_ps: per-step-states variant of the proven gdn_scan megakernel (f57590f).
// Adds states_out: after EACH step t's state update, writes the full state
// [t][h][v][k] — the per-step states the stash would have provided, as direct
// kernel outputs (bypasses the faulting stash path entirely).
#define HEADS 32
#define V_DIM 128
#define K_DIM 128
#define T_MAX 8
...
extern "C" __global__ void __launch_bounds__(256) gdn_scan_ps(
    ...
    float* __restrict__ states_out,   // [T][HEADS][V_DIM][K_DIM] per-step states
    ...
    const int Td)
{
  ...
  for (int vv = 0; vv < ROWS_PER_WARP; vv++) {
    ...
    for (int t = 0; t < T; t++) {
      ...
      kd = __shfl_sync(0xffffffffu, kd, 0);
      ...
      // per-step state write (THE route-3 addition)
      ...
      qd = __shfl_sync(0xffffffffu, qd, 0);
