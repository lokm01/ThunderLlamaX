...
#include <cuda_fp16.h>
extern "C" __global__ void __launch_bounds__(32) r_544_32_3_20_8_4_2_4(float* data0_52224, float* data1_15360, float* data2_3, unsigned char* data3_20480, unsigned char* data4_34119680, float* data5_1024) {
  float buf0[3];
  ...
  *(buf0+2) = 0.0f;
  for (int Ridx1_0_0 = 0; Ridx1_0_0 < 20; Ridx1_0_0++) {
    ...
    unsigned char val0 = (*(data4_34119680+(alu3+1)));
    unsigned char val1 = (*(data4_34119680+alu3));
    for (int Ridx1_0_1_0 = 0; Ridx1_0_1_0 < 8; Ridx1_0_1_0++) {
      int alu4 = (alu3+(Ridx1_0_1_0<<2));
      unsigned char val2 = (*(data4_34119680+(alu4+66)));
      unsigned char val3 = (*(data4_34119680+(alu4+67)));
      unsigned char val4 = (*(data4_34119680+(alu4+68)));
      unsigned char val5 = (*(data4_34119680+(alu4+69)));
      ...
      for (int Ridx1_0_1_1 = 0; Ridx1_0_1_1 < 4; Ridx1_0_1_1++) {
        ...
        for (int Ridx1_1_0 = 0; Ridx1_1_0 < 2; Ridx1_1_0++) {
          ...
          unsigned char val22 = (*(data4_34119680+(alu3+(Ridx1_0_1_0<<3)+(Ridx1_0_1_1<<1)+Ridx1_1_0+2)));
  ...
  float val29 = (*(buf0+1));
  float val30 = (*(buf0+2));
  float2 val31 = (*((float2*)((data2_3+0))));
  int alu50 = (lidx0+(gidx0<<5));
  *(data0_52224+alu50) = (val28*(1/(val31.x*(1.0f+exp2((val28*(1/val31.x)*-1.4426950216293335f))))));
  *(data0_52224+(alu50+17408)) = (val29*(1/(val31.y*(1.0f+exp2((val29*(1/val31.y)*-1.4426950216293335f))))));
  *(data0_52224+(alu50+34816)) = (val30*(1/(val27*(1.0f+exp2((val30*(1/val27)*-1.4426950216293335f))))));
}