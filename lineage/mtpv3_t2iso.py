...
"""Isolate the lazy T=3 replay hang: per-cycle prints, no eager assigns."""
import os, sys, time
...
toks = tt[:, pos:pos+2].contiguous().realize()

t0 = time.time()
with Context(JIT=1):
    logits = probe_j(toks, v_sp.bind(pos)).realize()
Device["NV"].synchronize()
print(f"[warm] capture+run OK {time.time()-t0:.1f}s", flush=True)
...
for i in range(N):
    ...
    with Context(JIT=1):
        logits = probe_j(toks, v_sp.bind(pos)).realize()
    Device["NV"].synchronize()
    print(f"[cyc {i}] replay OK {time.time()-t0:.3f}s", flush=True)
    pos += 2
print("ISO PASS", flush=True)
