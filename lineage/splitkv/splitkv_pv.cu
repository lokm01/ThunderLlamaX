...
// PV replacement for r_24_16_16_3_... : coalesced streaming, same 12-arg signature.
// CTA=(row,blockgrp) x 256 threads; thread t owns output dim t; positions streamed
// in lockstep (V reads coalesced across t). LMAX baked per-render.
#include <cuda_fp16.h>
extern "C" __global__ void __launch_bounds__(128) pv_coalesced(
...
{
  const int r = blockIdx.x, g = blockIdx.y, d = threadIdx.x;
  if (r == 0 && g == 0 && d == 0) data0[0] = 777.0f;   // SENTINEL
  ...
  for (int p = 0; p < LMAX; ++p)
    acc += exp2f((P_g[p] - mx) * 1.4426950216293335f) * __half2float(V_g[((size_t)p << 8) + d]);
  const float gate = data11[d + ((size_t)g << 9) + 256 + (size_t)r * 12288];
  data0[d + ((size_t)g << 8) + (size_t)r * 6144] =
    (acc / rs) * (1.0f / (1.0f + exp2f(-gate * 1.4426950216293335f)));
}
