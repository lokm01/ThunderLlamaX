...
#define A3B_BATCH 1
extern "C" __global__ void r_544_8_4_4_4_20_2_2_4_a3b(float* data0_17408, float* data1_17408, float* data2_5120, float* data3_1, unsigned char* data4_20480, unsigned char* data5_34119680, float* data6_1024, unsigned char* data7_11141120) {
  float* a3b_y = data0_17408;
  const float* a3b_x = data2_5120;
  const float* a3b_s = data3_1;
  const float* a3b_nw = (const float*)data4_20480;
  const float* a3b_aux = data1_17408;
  const unsigned char* a3b_W = data5_34119680;
  const float* a3b_g = data6_1024;
...
// Placeholders for the x-load, epilogue and warps-per-CTA count are filled by the fork hook.
  ...
  if (lane == 0) {
    a3b_y[warp] = a3b_aux[warp] * acc * (1.0f/a3b_s[warp/A3B_N]);
  }
