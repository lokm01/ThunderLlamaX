...
extern "C" __global__ void __launch_bounds__(256) k1_t2048(float* data0_147456, float* data1_18432, half* data2_4194304, const int data3_) {
  float* __restrict__ ws = (float*)0x1234567890ULL;
  const int s  = blockIdx.x / 12;
  const int gr = blockIdx.x % 12;
  const int g = gr / 3, r = gr % 3;
  const int start = s * 103;
  const int cend = min(start + 103, 2048);
  const int end_r = min(cend, data3_ + r + 1);
  const int h0 = g * 6;
  const __half* Kg = data2_4194304 + (size_t)g * (2048 * 256);
  const __half* Vg = Kg + (size_t)2048 * 1024;
  ...
  for (int t0 = start; t0 < cend; t0 += 32) {
    ...
    for (int pi = warp; pi < npos; pi += nwarp) {
      ...
      for (int h6 = 0; h6 < 6; h6++) {
        ...
        if (lane == 0) data0_147456[(size_t)(h0 + h6) * (3 * 2048) + (size_t)r * 2048 + p] = valid ? raw : NEG_INF;
        if (valid) {
