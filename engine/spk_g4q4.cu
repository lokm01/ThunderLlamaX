...
#define G4_QKROW(R, Mv, Sv, A8, PR) { \
  ...
  for (int c = 0; c < 32; ++c) { \
    ...
    const float2 kA = __half22float2(hk[0]), kB = __half22float2(hk[1]), kC = __half22float2(hk[2]), kD = __half22float2(hk[3]); \
    const float4 qa = *(const float4*)(qb + c*8); \
    const float4 qcx = *(const float4*)(qb + c*8 + 4); \
    sc0 += qa.x * kA.x; sc1 += qa.y * kA.y; sc2 += qa.z * kB.x; sc3 += qa.w * kB.y; \
    sc0 += qcx.x * kC.x; sc1 += qcx.y * kC.y; sc2 += qcx.z * kD.x; sc3 += qcx.w * kD.y; \
  } \
