    ...
    float* __restrict__ x4)
{
  ...
  const int toks[7] = { min(max(s0[0], 0), VOCAB - 1), min(max(s1[0], 0), VOCAB - 1), min(max(s2[0], 0), VOCAB - 1), min(max(s3[0], 0), VOCAB - 1), min(max(s4[0], 0), VOCAB - 1), min(max(s5[0], 0), VOCAB - 1), min(max(s6[0], 0), VOCAB - 1) };
  for (int t = 0; t < 7; ++t) {
