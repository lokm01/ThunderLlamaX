...
// R8 K=10 accept11k: the DEEP (K=10) accept — m-ladder to 10 over dring0..9.
// 22-word emit record: {pos_new, m, tok0..10, stop, cyc, hit, rsv...}; DecodeSession
// reads layout-aware (22+ when the K=10 deep set ran this cycle). emit[15] = l_hist[cyc]
// (9 = 8-gram hit) = the selection flag. h_seed = xA[m*DIM] (m <= 10 — T=11 rows).
// dhd_seed law unchanged (draft numerics carry no exactness contract).
#include <cuda_fp16.h>
#define DIM 5120
extern "C" __global__ void __launch_bounds__(256) accept11k(
    ...
    const int* __restrict__ dring8, const int* __restrict__ dring9,
    ...
    float* __restrict__ dhd_seed, int* __restrict__ emit, const int* __restrict__ l_hist)
{
  ...
  if (threadIdx.x == 0) {
      ...
      if (amds[1] == dring1[0]) { m = 2;
        if (amds[2] == dring2[0]) { m = 3;
          if (amds[3] == dring3[0]) { m = 4;
            ...
            if (amds[8] == dring8[0]) { m = 9;
            if (amds[9] == dring9[0]) m = 10; } } } } } } } } }
    ...
    emit[10] = amds[8]; emit[11] = amds[9]; emit[12] = amds[10];
    emit[13] = 0;                   // stop_flag
    emit[14] = cyc;
    emit[15] = l_hist[cyc];         // hit flag (9 = 8-gram hit)
    cyc_slot[0] = cyc + 1;
    ms = m;
  }
  __syncthreads();
