...
// R5 K=6 acceptsel7k: deep-set state select. [48][5] rec4/conv4 layout and
// live=slot-4 PRESERVED. rec: m<5 slot m -> 4; m==5 rec6x; m==6 rec7x.
// conv: m<4 slot m; m==4 conv5x; m==5 conv6x; m==6 conv7x.
#include <cuda_fp16.h>
extern "C" __global__ void __launch_bounds__(256) acceptsel7k(
    float* __restrict__ rec4, float* __restrict__ conv4, const int* __restrict__ m_slot,
    const float* __restrict__ conv5x, const float* __restrict__ conv6x, const float* __restrict__ conv7x,
    const float* __restrict__ rec6x, const float* __restrict__ rec7x)
{
