...
// R8 K=9 accept10k: the DEEP (K=9) accept — m-ladder to 9 over dring0..8.
// 21-word emit record: {pos_new, m, tok0..9, stop, cyc, hit, rsv...}; DecodeSession
// reads layout-aware (21+ when the K=9 deep set ran this cycle). emit[14] = l_hist[cyc]
// (9 = 8-gram hit) = the selection flag. h_seed = xA[m*DIM] (m <= 9 — T=10 rows).
// dhd_seed law unchanged (draft numerics carry no exactness contract).
#include <cuda_fp16.h>
#define DIM 5120
extern "C" __global__ void __launch_bounds__(256) accept10k(
    ...
    float* __restrict__ dhd_seed, int* __restrict__ emit, const int* __restrict__ l_hist)
{
  ...
  if (threadIdx.x == 0) {
    ...
    emit[10] = amds[8]; emit[11] = amds[9];
    emit[12] = 0;                   // stop_flag
    emit[13] = cyc;
    emit[14] = l_hist[cyc];         // hit flag (9 = 8-gram hit)
    cyc_slot[0] = cyc + 1;
    ms = m;
  }
  __syncthreads();
