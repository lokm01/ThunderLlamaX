...
extern "C" __global__ void __launch_bounds__(NTHR) KNAME(
  ...
  uint32_t acc = 0;
  const int n = u1 - u0;
  const int steps = n >> 5;          // 32 lanes x consecutive uint4 per step
  for (int rep = 0; rep < reps; ++rep) {
    _Pragma("unroll 8")
    for (int s = 0; s < steps; ++s) {
      const int u = u0 + (s << 5) + lane;
    ...
    {  // tail: the <32 leftover units of the last warp
      const int u = u0 + (steps << 5) + lane;
      if (u < u1) {
        const uint4 v = base[u];
#if !P7A_NODP
        int dp = __dp4a((int)v.x, q8, 0);
