...
// R5d K=7 LOOKUP8 drafter: deep-K (K=7) n-gram proposal kernel.
// Same contract as lookup7_nw32 (R5c) with the DEEP-K SCAN-RANGE LAW at K=7:
// iend = pos - 8 - K = pos - 15 (all 7 proposal tokens hist[i+8..i+14] are
// WRITTEN ids <= hist[pos-1] = cur). Writes dring0..6 only on hit (l >= 8);
...
extern "C" __global__ void __launch_bounds__(1024) lookup8_nw32(
    ...
    int* __restrict__ l_hist, const int* __restrict__ cyc_slot)
{
  ...
  int lkey = 0;
  if (pos >= 15) {   // suffix defined (pos>=8) and scan [0,pos-15] non-empty
  ...
  if (tid == 0) {
    const int key = bkey;
    const int l = key >> 20;
    l_hist[cyc_slot[0]] = (pos < 15) ? (1000000 + pos) : (l + 1);   // 1..8 miss; 9 hit
    if (l >= 8) {
