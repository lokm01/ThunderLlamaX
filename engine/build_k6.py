...
"""R5 K=6: build the M=7/T=7 kernel set (mirrors build_k5.py)."""
...
for n in ("lookup7_nw32", "accept7k", "acceptsel7k"):
    build(n)

C100, S = 100352, 256
build("spk_pre7qh_100k", src="spk_preqh", extra=("-DKPRE=spk_pre7qh_100k", f"-DCTXK={C100}", "-DROWS=7"))
# hm7: tuned variant for the 64-reg/1024-thr budget (ks4 unroll; tune below if spill)
_g7 = open(f"{BASE}/spk_g4hm.cu").read()
_prag = chr(95) + chr(80) + 'ragma("' + "unroll" + '")' + chr(10) + "        for (int ks4 = 0; ks4 < QK_KS; ++ks4) {"
assert _prag in _g7
open(f"{BASE}/.spk_g4hm_r7.cu", "w").write(_g7.replace(_prag, "#pragma unroll 4" + chr(10) + "        for (int ks4 = 0; ks4 < QK_KS; ++ks4) {", 1))
build("spk_g4nw32hm7_100k", src=".spk_g4hm_r7", extra=(f"-DKNAME=spk_g4nw32hm7_100k", f"-DCTXK={C100}", "-DROWS=7",
...
_h, _b = split_skv()
open(f"{BASE}/spk_c7g_100k.cu", "w").write(_h + _b["K2S"])
build("spk_c7g_100k", extra=("-DK2S=spk_c7g_100k", f"-DCTXK={C100}", "-DROWS=7", f"-DS={S}", f"-DCH={C100//S}", "-DUNROLL=4"))
print("[build k6 done]")
