...
# ================= P13 kernel B minimal: bench-only, 4 new bufs, direct bases =================
NBLK = 8
WB = 2 * 17408 * 1960
R7F = 512.0 / 392.0
...
P.poison("om8", NBLK * 32 * 17408, np.float16, 7.7)
dev.synchronize()
print("[p13b] 4 bufs up", flush=True)
import time as _t
def bench(fn, n=10):
  fn(); dev.synchronize()
  best = 1e9
...
for kn in ("pf13ffn_w2_nw8", "pf13ffn_w4_nw8", "pf13ffn_w8_nw8"):
  prp = prog(kn)
  prp(P.d["w1c"], P.d["w2c"], P.d["gridf"], P.d["x8"], P.d["om8"], global_size=(82, 1, 1), local_size=LS)
  dev.synchronize()
  o = P.down("om8", (NBLK * 32, 17408), np.float16)
  print(f"[p13b-run] {kn}: RAN, out absmax {float(np.abs(o.astype(np.float32)).max()):.1f}, nnz {int((np.abs(o.astype(np.float32))>1e-3).sum())}", flush=True)
  def one():
    prp(P.d["w1c"], P.d["w2c"], P.d["gridf"], P.d["x8"], P.d["om8"], global_size=(82, 1, 1), local_size=LS)
  t = bench(one) / NBLK
  print(f"[p13b-bench] {kn}: {t*1e6:7.1f} us/blk | w {WB/t/1e9:6.1f} | amort {2*WB/t/1e9:6.1f} | dram {R7F*WB/t/1e9:6.1f} GB/s | gate {'PASS' if 2*WB/t/1e9 >= 350 else 'FAIL'}", flush=True)
