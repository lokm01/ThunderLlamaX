...
// ---- h_embed4 ----
extern "C" __global__ void __launch_bounds__(256) h_embed7(
    const unsigned char* __restrict__ emb, const float* __restrict__ grid512,
    const int* __restrict__ s0, const int* __restrict__ s1, const int* __restrict__ s2, const int* __restrict__ s3, const int* __restrict__ s4, const int* __restrict__ s5, const int* __restrict__ s6,
    float* __restrict__ x4)
{
  const int tid = threadIdx.x;
  const int toks[7] = { s0[0], s1[0], s2[0], s3[0], s4[0], s5[0], s6[0] };
  for (int t = 0; t < 7; ++t) {
...
// ---- k0n4 ----
extern "C" __global__ void __launch_bounds__(256) k0n7(
...
// ---- k0ab4 ----
extern "C" __global__ void __launch_bounds__(256) k0ab7(
...
extern "C" __global__ void __launch_bounds__(256) k2s7(
    float* __restrict__ conv_b, float* __restrict__ rec_b, float* __restrict__ conv5x, float* __restrict__ conv6x, float* __restrict__ conv7x, float* __restrict__ rec6x, float* __restrict__ rec7x,
    ...
    float* __restrict__ core, const float* __restrict__ snw, __half* __restrict__ z4)
{
  ...
  for (int t = 0; t < 7; ++t) {
    ...
    }
    {
      ...
      float* dst = (t == 4) ? conv5x : (t == 5) ? conv6x : (t == 6) ? conv7x : (conv_b + (size_t)t * (3*CONV_CH));
...
// ---- op38nw32_4 ----
extern "C" __global__ void __launch_bounds__(1024) op38nw32_7(
    ...
    const __half* __restrict__ z4, __half* __restrict__ attn_out4)
{
  ...
  for (int b = 0; b < 24; ++b) {
    ...
    const float4 g1 = *((const float4*)(gridf + ((q >> 8) << 2)));
    const int koff = (b << 8) + (lane << 3);
    LDH2(xv0, z4, 6144, 0, koff) LDH2(xv1, z4, 6144, 1, koff) LDH2(xv2, z4, 6144, 2, koff) LDH2(xv3, z4, 6144, 3, koff) LDH2(xv4, z4, 6144, 4, koff) LDH2(xv5, z4, 6144, 5, koff) LDH2(xv6, z4, 6144, 6, koff)
    const float sg0 = ((sidx>>0)&1) ? -1.f : 1.f, sg1 = ((sidx>>1)&1) ? -1.f : 1.f;
    const float sg2 = ((sidx>>2)&1) ? -1.f : 1.f, sg3 = ((sidx>>3)&1) ? -1.f : 1.f;
    const float sg4 = ((sidx>>4)&1) ? -1.f : 1.f, sg5 = ((sidx>>5)&1) ? -1.f : 1.f;
    const float sg6 = ((sidx>>6)&1) ? -1.f : 1.f, sg7 = spar ? -1.f : 1.f;
  ...
  RED7(a0,a1,a2,a3,a4,a5,a6)
  if (lane == 0) { attn_out4[0*DIM+warp] = (__half)a0; attn_out4[1*DIM+warp] = (__half)a1; attn_out4[2*DIM+warp] = (__half)a2; attn_out4[3*DIM+warp] = (__half)a3; attn_out4[4*DIM+warp] = (__half)a4; attn_out4[5*DIM+warp] = (__half)a5; attn_out4[6*DIM+warp] = (__half)a6; }
}

// ---- k3aonw32_4 (Q8_0) ----
extern "C" __global__ void __launch_bounds__(1024) k3aonw32_7(
...
// ---- ao8nw32_4 (IQ3_S raw) ----
extern "C" __global__ void __launch_bounds__(1024) ao8nw32_7(
    ...
    const __half* __restrict__ ao_in4, __half* __restrict__ attn_out4)
{
  ...
  for (int b = 0; b < 24; ++b) {
    ...
    const int koff = (b << 8) + (lane << 3);
    LDH2(xv0, ao_in4, 6144, 0, koff) LDH2(xv1, ao_in4, 6144, 1, koff) LDH2(xv2, ao_in4, 6144, 2, koff) LDH2(xv3, ao_in4, 6144, 3, koff) LDH2(xv4, ao_in4, 6144, 4, koff) LDH2(xv5, ao_in4, 6144, 5, koff) LDH2(xv6, ao_in4, 6144, 6, koff)
...
// ---- hh4 ----
extern "C" __global__ void __launch_bounds__(256) hh7(
...
// ---- ffn8v4 ----
extern "C" __global__ void __launch_bounds__(256) ffn8v7(
    ...
    const float* __restrict__ gridf, const __half* __restrict__ hhx4, __half* __restrict__ gact4)
{
  ...
  float ag0=0.f,ag1=0.f,ag2=0.f,ag3=0.f,ag4=0.f,ag5=0.f,ag6=0.f, au0=0.f,au1=0.f,au2=0.f,au3=0.f,au4=0.f,au5=0.f,au6=0.f;
  #pragma unroll 5
  for (int b = 0; b < 20; ++b) {
    const int koff = (b << 8) + (lane << 3);
    LDH2(xg0, hhx4, DIM, 0, koff) LDH2(xg1, hhx4, DIM, 1, koff) LDH2(xg2, hhx4, DIM, 2, koff) LDH2(xg3, hhx4, DIM, 3, koff) LDH2(xg4, hhx4, DIM, 4, koff) LDH2(xg5, hhx4, DIM, 5, koff) LDH2(xg6, hhx4, DIM, 6, koff)
    #define IQ3V7(QP, SP, DP, A0, A1, A2, A3, A4, A5, A6) { \
      ...
      const float sg6 = ((sidx>>6)&1) ? -1.f : 1.f, sg7 = spar ? -1.f : 1.f; \
      float wv[8] = { db*g0.x*sg0, db*g0.y*sg1, db*g0.z*sg2, db*g0.w*sg3, db*g1.x*sg4, db*g1.y*sg5, db*g1.z*sg6, db*g1.w*sg7 }; \
      ACC7H2(xg0, xg1, xg2, xg3, xg4, xg5, xg6, wv, A0, A1, A2, A3, A4, A5, A6) }
    IQ3V7(qg, sg, dg, ag0, ag1, ag2, ag3, ag4, ag5, ag6)
    IQ3V7(qu, su, du, au0, au1, au2, au3, au4, au5, au6)
    #undef IQ3V4
  }
  RED7(ag0,ag1,ag2,ag3,ag4,ag5,ag6)
  RED7(au0,au1,au2,au3,au4,au5,au6)
  if (lane == 0) {
    ...
    gact4[3*FFN_N + warp] = __hmul(hsilu_h4((__half)ag3), (__half)au3); gact4[4*FFN_N + warp] = __hmul(hsilu_h4((__half)ag4), (__half)au4); gact4[5*FFN_N + warp] = __hmul(hsilu_h4((__half)ag5), (__half)au5); gact4[6*FFN_N + warp] = __hmul(hsilu_h4((__half)ag6), (__half)au6);
  }
}

// ---- down8nw32_4 ----
extern "C" __global__ void __launch_bounds__(1024) down8nw32_7(
    ...
    const __half* __restrict__ gact4, const float* __restrict__ hh4b, float* __restrict__ y4)
{
  ...
  for (int b = 0; b < 68; ++b) {
    ...
    const float4 g1 = *((const float4*)(gridf + ((q >> 8) << 2)));
    const int koff = (b << 8) + (lane << 3);
    LDH2(xv0, gact4, FFN_N, 0, koff) LDH2(xv1, gact4, FFN_N, 1, koff) LDH2(xv2, gact4, FFN_N, 2, koff) LDH2(xv3, gact4, FFN_N, 3, koff) LDH2(xv4, gact4, FFN_N, 4, koff) LDH2(xv5, gact4, FFN_N, 5, koff) LDH2(xv6, gact4, FFN_N, 6, koff)
    const float sg0 = ((sidx>>0)&1) ? -1.f : 1.f, sg1 = ((sidx>>1)&1) ? -1.f : 1.f;
    const float sg2 = ((sidx>>2)&1) ? -1.f : 1.f, sg3 = ((sidx>>3)&1) ? -1.f : 1.f;
    const float sg4 = ((sidx>>4)&1) ? -1.f : 1.f, sg5 = ((sidx>>5)&1) ? -1.f : 1.f;
    const float sg6 = ((sidx>>6)&1) ? -1.f : 1.f, sg7 = spar ? -1.f : 1.f;
  ...
  if (lane == 0) {
    ...
    y4[3*DIM+warp] = hh4b[3*DIM+warp] + (float)((__half)a3); y4[4*DIM+warp] = hh4b[4*DIM+warp] + (float)((__half)a4); y4[5*DIM+warp] = hh4b[5*DIM+warp] + (float)((__half)a5); y4[6*DIM+warp] = hh4b[6*DIM+warp] + (float)((__half)a6);
  }
}

// ---- aq3k8v4 (q IQ3 packed + k IQ3 packed + v Q4_K raw) ----
extern "C" __global__ void __launch_bounds__(256) aq3k8v7(
  ...
  if (warp < 12288) {
    #define AQI3P4(ROWB, OB, OS, RIDX) { \
      ...
      _Pragma("unroll 5") for (int b = 0; b < 20; ++b) { \
        ...
        ACC7H2(xv0, xv1, xv2, xv3, xv4, xv5, xv6, wv, a0, a1, a2, a3, a4, a5, a6) } \
      RED7(a0,a1,a2,a3,a4,a5,a6) \
      if (lane == 0) { (OB)[0*(OS)+(RIDX)] = (__half)a0; (OB)[1*(OS)+(RIDX)] = (__half)a1; (OB)[2*(OS)+(RIDX)] = (__half)a2; (OB)[3*(OS)+(RIDX)] = (__half)a3; (OB)[4*(OS)+(RIDX)] = (__half)a4; (OB)[5*(OS)+(RIDX)] = (__half)a5; (OB)[6*(OS)+(RIDX)] = (__half)a6; } }  // R5 fix: the row-4 store was MISSING (gen_m5 slip) -> qrow3 row 4 stale -> wrong amds[4]
    ...
    #define AKI3(ROWB) { \
      ...
      _Pragma("unroll 5") for (int b = 0; b < 20; ++b) { \
        ...
        ACC7H2(xv0, xv1, xv2, xv3, xv4, xv5, xv6, wv, a0, a1, a2, a3, a4, a5, a6) } \
      RED7(a0,a1,a2,a3,a4,a5,a6) \
      if (lane == 0) { krow4[0*1024+r] = (__half)a0; krow4[1*1024+r] = (__half)a1; krow4[2*1024+r] = (__half)a2; krow4[3*1024+r] = (__half)a3; krow4[4*1024+r] = (__half)a4; krow4[5*1024+r] = (__half)a5; krow4[6*1024+r] = (__half)a6; } }
    ...
    RED7(a0,a1,a2,a3,a4,a5,a6)
    if (lane == 0) { vrow4[0*1024+r] = (__half)a0; vrow4[1*1024+r] = (__half)a1; vrow4[2*1024+r] = (__half)a2; vrow4[3*1024+r] = (__half)a3; vrow4[4*1024+r] = (__half)a4; vrow4[5*1024+r] = (__half)a5; vrow4[6*1024+r] = (__half)a6; }
  }
}

// ---- head8v4 ----
extern "C" __global__ void __launch_bounds__(256) head8v7(
    const unsigned char* __restrict__ wq5, const __half* __restrict__ xh4, __half* __restrict__ logits4)
{
  ...
  RED7(a0,a1,a2,a3,a4,a5,a6)
  if (lane == 0) { logits4[0*VOCAB+warp] = (__half)a0; logits4[1*VOCAB+warp] = (__half)a1; logits4[2*VOCAB+warp] = (__half)a2; logits4[3*VOCAB+warp] = (__half)a3; logits4[4*VOCAB+warp] = (__half)a4; logits4[5*VOCAB+warp] = (__half)a5; logits4[6*VOCAB+warp] = (__half)a6; }
}

// ---- aq6k8v4 (q Q6_K packed + k IQ3 packed + v Q4_K raw) ----
extern "C" __global__ void __launch_bounds__(256) aq6k8v7(
    const unsigned char* __restrict__ wq, const unsigned char* __restrict__ wk, const unsigned char* __restrict__ wv4,
    const float* __restrict__ gridf, const __half* __restrict__ xh4,
    __half* __restrict__ qrow4, __half* __restrict__ krow4, __half* __restrict__ vrow4)
{
  ...
  if (warp < 12288) {
    ...
    RED7(a0,a1,a2,a3,a4,a5,a6)
    if (lane == 0) { qrow4[0*12288+warp] = (__half)a0; qrow4[1*12288+warp] = (__half)a1; qrow4[2*12288+warp] = (__half)a2; qrow4[3*12288+warp] = (__half)a3; qrow4[4*12288+warp] = (__half)a4; qrow4[5*12288+warp] = (__half)a5; qrow4[6*12288+warp] = (__half)a6; }
  } else if (warp < 13312) {
    const int r = warp - 12288;
