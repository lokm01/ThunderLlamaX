...
#define PIN16() unsigned z0=threadIdx.x, z1=threadIdx.x^1, z2=threadIdx.x^2, z3=threadIdx.x^3, \
    z4=threadIdx.x^4, z5=threadIdx.x^5, z6=threadIdx.x^6, z7=threadIdx.x^7, \
    z8=threadIdx.x^8, z9=threadIdx.x^9, z10=threadIdx.x^10, z11=threadIdx.x^11, \
    z12=threadIdx.x^12, z13=threadIdx.x^13, z14=threadIdx.x^14, z15=threadIdx.x^15; \
  asm volatile("" : "+r"(z0), "+r"(z1), "+r"(z2), "+r"(z3), "+r"(z4), "+r"(z5), "+r"(z6), "+r"(z7), \
                     "+r"(z8), "+r"(z9), "+r"(z10), "+r"(z11), "+r"(z12), "+r"(z13), "+r"(z14), "+r"(z15))
#define CONSPIN() (z0+z1+z2+z3+z4+z5+z6+z7+z8+z9+z10+z11+z12+z13+z14+z15)
...
extern "C" __global__ void __launch_bounds__(T) e1a_ldg_nw8(const uint4* __restrict__ src, unsigned* out)
{
  ...
  if ((acc ^ (unsigned)CONSPIN()) == 0xdeadbeefu) out[blockIdx.x] = acc;
}

// ---------------- ARM A0: clock-windowed twin of A (per-CTA cycles) ----------------
extern "C" __global__ void __launch_bounds__(T) e1a0_ldg_nw8(const uint4* __restrict__ src, unsigned* out)
{
  ...
  if (threadIdx.x == 0) { out[o+2] = (unsigned)clock64(); out[o+3] = acc ^ (unsigned)CONSPIN(); }
}
...
extern "C" __global__ void __launch_bounds__(T) e1b_fma_nw8(const uint4* __restrict__ src, unsigned* out)
{
  ...
  const unsigned u = (unsigned)(a0 + a1 + a2 + a3) ^ (unsigned)CONSPIN();
...
extern "C" __global__ void __launch_bounds__(T) e1c_hmma_nw8(const uint4* __restrict__ src, unsigned* out)
{
  ...
  const unsigned u = (unsigned)(c0 + c1 + c2 + c3 + d0 + d1 + d2 + d3) ^ (unsigned)CONSPIN();
  if (u == 0xdeadbeefu) out[blockIdx.x] = u;
}

// ---------------- ARM D: production mix (2 LDG + decode ALU + 1 HMMA) ----------------
extern "C" __global__ void __launch_bounds__(T) e1d_mix_nw8(const uint4* __restrict__ src, unsigned* out)
{
  ...
  const unsigned u = ((unsigned)(c0 + c1 + c2 + c3) + acc) ^ (unsigned)CONSPIN();
  if (u == 0xdeadbeefu) out[blockIdx.x] = u;
}
...
extern "C" __global__ void __launch_bounds__(T) e1e_copy_nw8(const uint4* __restrict__ src, uint4* __restrict__ dst, unsigned* out)
{
  ...
  if ((g ^ (unsigned)CONSPIN()) == 0xdeadbeefu) out[blockIdx.x] = g;
}

// ---------------- ARM F: dependent loads (latency bracket) ----------------
extern "C" __global__ void __launch_bounds__(T) e1f_dldg_nw8(const uint4* __restrict__ src, unsigned* out)
{
  ...
  for (int it = 0; it < ITER_F; ++it) {
    v = base[((v.x ^ v.y ^ acc) & 0x3FFFFu)];
    v = base[((v.z ^ v.w ^ acc) & 0x3FFFFu)];
    acc ^= v.x + v.y + v.z + v.w;
  }
  if ((acc ^ (unsigned)CONSPIN()) == 0xdeadbeefu) out[blockIdx.x] = acc;
}
