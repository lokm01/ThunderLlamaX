...
extern "C" __global__ void __launch_bounds__(256) h_embed9(
    const unsigned char* __restrict__ emb, const float* __restrict__ grid512,
    const int* __restrict__ s0, const int* __restrict__ s1, const int* __restrict__ s2, const int* __restrict__ s3, const int* __restrict__ s4, const int* __restrict__ s5, const int* __restrict__ s6, const int* __restrict__ s7, const int* __restrict__ s8,
    float* __restrict__ x4)
{
  const int tid = threadIdx.x;
  const int t = blockIdx.x;
  const int toks[9] = { s0[0], s1[0], s2[0], s3[0], s4[0], s5[0], s6[0], s7[0], s8[0] };
  {
...
extern "C" __global__ void __launch_bounds__(256) k0n9(
...
extern "C" __global__ void __launch_bounds__(256) k0ab9(
...
// ---- q5g8v4 ----
extern "C" __global__ void __launch_bounds__(256) q5g8v9(
    ...
    __half* __restrict__ qkv4, __half* __restrict__ gate4)
{
  ...
  if (warp < 10240) {
    ...
    _Pragma("unroll 5") for (int b = 0; b < 20; ++b) {
      ...
      LDH2(xv0, xh4, DIM, 0, koff) LDH2(xv1, xh4, DIM, 1, koff) LDH2(xv2, xh4, DIM, 2, koff) LDH2(xv3, xh4, DIM, 3, koff) LDH2(xv4, xh4, DIM, 4, koff) LDH2(xv5, xh4, DIM, 5, koff) LDH2(xv6, xh4, DIM, 6, koff) LDH2(xv7, xh4, DIM, 7, koff) LDH2(xv8, xh4, DIM, 8, koff)
      float wv[8];
    ...
    if (lane == 0) { qkv4[0*10240+warp] = (__half)a0; qkv4[1*10240+warp] = (__half)a1; qkv4[2*10240+warp] = (__half)a2; qkv4[3*10240+warp] = (__half)a3; qkv4[4*10240+warp] = (__half)a4; qkv4[5*10240+warp] = (__half)a5; qkv4[6*10240+warp] = (__half)a6; qkv4[7*10240+warp] = (__half)a7; qkv4[8*10240+warp] = (__half)a8; }
  } else {
    ...
    RED9(a0,a1,a2,a3,a4,a5,a6,a7,a8)
    if (lane == 0) { gate4[0*6144+r] = (__half)a0; gate4[1*6144+r] = (__half)a1; gate4[2*6144+r] = (__half)a2; gate4[3*6144+r] = (__half)a3; gate4[4*6144+r] = (__half)a4; gate4[5*6144+r] = (__half)a5; gate4[6*6144+r] = (__half)a6; gate4[7*6144+r] = (__half)a7; gate4[8*6144+r] = (__half)a8; }
  }
}

// ---- k2s4: conv+silu+L2+delta-scan 4 steps, per-step slots 0..3 + live 4 ----
extern "C" __global__ void __launch_bounds__(256) k2s9(
    float* __restrict__ conv_b, float* __restrict__ rec_b, float* __restrict__ conv5x, float* __restrict__ conv6x, float* __restrict__ conv7x, float* __restrict__ conv8x, float* __restrict__ conv9x, float* __restrict__ rec6x, float* __restrict__ rec7x, float* __restrict__ rec8x, float* __restrict__ rec9x,
    ...
    float* __restrict__ core, const float* __restrict__ snw, __half* __restrict__ z4)
{
  ...
  for (int t = 0; t < 9; ++t) {
    ...
    float* rec_out = (t >= 5) ? (((t == 5) ? rec6x : (t == 6) ? rec7x : (t == 7) ? rec8x : rec9x) + (size_t)h*128*128)
    ...
    }
    {
      ...
      float* dst = (t == 4) ? conv5x : (t == 5) ? conv6x : (t == 6) ? conv7x : (t == 7) ? conv8x : (t == 8) ? conv9x : (conv_b + (size_t)t * (3*CONV_CH));
...
// ---- op38nw32_4 ----
extern "C" __global__ void __launch_bounds__(1024) op38nw32_9(
    ...
    const __half* __restrict__ z4, __half* __restrict__ attn_out4)
{
  ...
  for (int b = 0; b < 24; ++b) {
    ...
    const float4 g1 = *((const float4*)(gridf + ((q >> 8) << 2)));
    const int koff = (b << 8) + (lane << 3);
    LDH2(xv0, z4, 6144, 0, koff) LDH2(xv1, z4, 6144, 1, koff) LDH2(xv2, z4, 6144, 2, koff) LDH2(xv3, z4, 6144, 3, koff) LDH2(xv4, z4, 6144, 4, koff) LDH2(xv5, z4, 6144, 5, koff) LDH2(xv6, z4, 6144, 6, koff) LDH2(xv7, z4, 6144, 7, koff) LDH2(xv8, z4, 6144, 8, koff)
    const float sg0 = ((sidx>>0)&1) ? -1.f : 1.f, sg1 = ((sidx>>1)&1) ? -1.f : 1.f;
    const float sg2 = ((sidx>>2)&1) ? -1.f : 1.f, sg3 = ((sidx>>3)&1) ? -1.f : 1.f;
    const float sg4 = ((sidx>>4)&1) ? -1.f : 1.f, sg5 = ((sidx>>5)&1) ? -1.f : 1.f;
    const float sg6 = ((sidx>>6)&1) ? -1.f : 1.f, sg7 = spar ? -1.f : 1.f;
  ...
  RED9(a0,a1,a2,a3,a4,a5,a6,a7,a8)
  if (lane == 0) { attn_out4[0*DIM+warp] = (__half)a0; attn_out4[1*DIM+warp] = (__half)a1; attn_out4[2*DIM+warp] = (__half)a2; attn_out4[3*DIM+warp] = (__half)a3; attn_out4[4*DIM+warp] = (__half)a4; attn_out4[5*DIM+warp] = (__half)a5; attn_out4[6*DIM+warp] = (__half)a6; attn_out4[7*DIM+warp] = (__half)a7; attn_out4[8*DIM+warp] = (__half)a8; }
}

// ---- k3aonw32_4 (Q8_0) ----
extern "C" __global__ void __launch_bounds__(1024) k3aonw32_9(
...
// ---- ao8nw32_4 (IQ3_S raw) ----
extern "C" __global__ void __launch_bounds__(1024) ao8nw32_9(
    ...
    const __half* __restrict__ ao_in4, __half* __restrict__ attn_out4)
{
  ...
  for (int b = 0; b < 24; ++b) {
    ...
    const int koff = (b << 8) + (lane << 3);
    LDH2(xv0, ao_in4, 6144, 0, koff) LDH2(xv1, ao_in4, 6144, 1, koff) LDH2(xv2, ao_in4, 6144, 2, koff) LDH2(xv3, ao_in4, 6144, 3, koff) LDH2(xv4, ao_in4, 6144, 4, koff) LDH2(xv5, ao_in4, 6144, 5, koff) LDH2(xv6, ao_in4, 6144, 6, koff) LDH2(xv7, ao_in4, 6144, 7, koff) LDH2(xv8, ao_in4, 6144, 8, koff)
...
// R7a norms rung: one ROW per CTA (t = blockIdx.x, grid 8; was grid 1 with a
// serial 8-row t-loop). Per-row math VERBATIM -> bit-identical.
extern "C" __global__ void __launch_bounds__(256) hh9(
...
// ---- ffn8v4 ----
extern "C" __global__ void __launch_bounds__(256) ffn8v9(
    ...
    const float* __restrict__ gridf, const __half* __restrict__ hhx4, __half* __restrict__ gact4)
{
  ...
  float ag0=0.f,ag1=0.f,ag2=0.f,ag3=0.f,ag4=0.f,ag5=0.f,ag6=0.f,ag7=0.f,ag8=0.f, au0=0.f,au1=0.f,au2=0.f,au3=0.f,au4=0.f,au5=0.f,au6=0.f,au7=0.f,au8=0.f;
  #pragma unroll 5
  for (int b = 0; b < 20; ++b) {
    const int koff = (b << 8) + (lane << 3);
    LDH2(xg0, hhx4, DIM, 0, koff) LDH2(xg1, hhx4, DIM, 1, koff) LDH2(xg2, hhx4, DIM, 2, koff) LDH2(xg3, hhx4, DIM, 3, koff) LDH2(xg4, hhx4, DIM, 4, koff) LDH2(xg5, hhx4, DIM, 5, koff) LDH2(xg6, hhx4, DIM, 6, koff) LDH2(xg7, hhx4, DIM, 7, koff) LDH2(xg8, hhx4, DIM, 8, koff)
    #define IQ3V9(QP, SP, DP, A0, A1, A2, A3, A4, A5, A6, A7, A8) { \
      ...
      const float sg6 = ((sidx>>6)&1) ? -1.f : 1.f, sg7 = spar ? -1.f : 1.f; \
      float wv[8] = { db*g0.x*sg0, db*g0.y*sg1, db*g0.z*sg2, db*g0.w*sg3, db*g1.x*sg4, db*g1.y*sg5, db*g1.z*sg6, db*g1.w*sg7 }; \
      ACC9H2(xg0, xg1, xg2, xg3, xg4, xg5, xg6, xg7, xg8, wv, A0, A1, A2, A3, A4, A5, A6, A7, A8) }
    IQ3V9(qg, sg, dg, ag0, ag1, ag2, ag3, ag4, ag5, ag6, ag7, ag8)
    IQ3V9(qu, su, du, au0, au1, au2, au3, au4, au5, au6, au7, au8)
    #undef IQ3V4
  }
  RED9(ag0,ag1,ag2,ag3,ag4,ag5,ag6,ag7,ag8)
  RED9(au0,au1,au2,au3,au4,au5,au6,au7,au8)
  if (lane == 0) {
...
extern "C" __global__ void __launch_bounds__(256) aq3k8v9(
  ...
  if (warp < 12288) {
    #define AQI3P4(ROWB, OB, OS, RIDX) { \
      ...
      _Pragma("unroll 2") for (int b = 0; b < 20; ++b) { \
        ...
        ACC9H2(xv0, xv1, xv2, xv3, xv4, xv5, xv6, xv7, xv8, wv, a0, a1, a2, a3, a4, a5, a6, a7, a8) } \
      RED9(a0,a1,a2,a3,a4,a5,a6,a7,a8) \
      if (lane == 0) { (OB)[0*(OS)+(RIDX)] = (__half)a0; (OB)[1*(OS)+(RIDX)] = (__half)a1; (OB)[2*(OS)+(RIDX)] = (__half)a2; (OB)[3*(OS)+(RIDX)] = (__half)a3; (OB)[4*(OS)+(RIDX)] = (__half)a4; (OB)[5*(OS)+(RIDX)] = (__half)a5; (OB)[6*(OS)+(RIDX)] = (__half)a6; (OB)[7*(OS)+(RIDX)] = (__half)a7; (OB)[8*(OS)+(RIDX)] = (__half)a8; } }  // R5 fix: the row-4 store was MISSING (gen_m5 slip) -> qrow3 row 4 stale -> wrong amds[4]
    ...
    #define AKI3(ROWB) { \
      ...
      _Pragma("unroll 2") for (int b = 0; b < 20; ++b) { \
        ...
        ACC9H2(xv0, xv1, xv2, xv3, xv4, xv5, xv6, xv7, xv8, wv, a0, a1, a2, a3, a4, a5, a6, a7, a8) } \
      RED9(a0,a1,a2,a3,a4,a5,a6,a7,a8) \
      if (lane == 0) { krow4[0*1024+r] = (__half)a0; krow4[1*1024+r] = (__half)a1; krow4[2*1024+r] = (__half)a2; krow4[3*1024+r] = (__half)a3; krow4[4*1024+r] = (__half)a4; krow4[5*1024+r] = (__half)a5; krow4[6*1024+r] = (__half)a6; krow4[7*1024+r] = (__half)a7; krow4[8*1024+r] = (__half)a8; } }
    ...
    RED9(a0,a1,a2,a3,a4,a5,a6,a7,a8)
    if (lane == 0) { vrow4[0*1024+r] = (__half)a0; vrow4[1*1024+r] = (__half)a1; vrow4[2*1024+r] = (__half)a2; vrow4[3*1024+r] = (__half)a3; vrow4[4*1024+r] = (__half)a4; vrow4[5*1024+r] = (__half)a5; vrow4[6*1024+r] = (__half)a6; vrow4[7*1024+r] = (__half)a7; vrow4[8*1024+r] = (__half)a8; }
  }
}

// ---- head8v4 ----
extern "C" __global__ void __launch_bounds__(256) head8v9(
    const unsigned char* __restrict__ wq5, const __half* __restrict__ xh4, __half* __restrict__ logits4)
{
  ...
  RED9(a0,a1,a2,a3,a4,a5,a6,a7,a8)
  if (lane == 0) { logits4[0*VOCAB+warp] = (__half)a0; logits4[1*VOCAB+warp] = (__half)a1; logits4[2*VOCAB+warp] = (__half)a2; logits4[3*VOCAB+warp] = (__half)a3; logits4[4*VOCAB+warp] = (__half)a4; logits4[5*VOCAB+warp] = (__half)a5; logits4[6*VOCAB+warp] = (__half)a6; logits4[7*VOCAB+warp] = (__half)a7; logits4[8*VOCAB+warp] = (__half)a8; }
}

// ---- aq6k8v4 (q Q6_K packed + k IQ3 packed + v Q4_K raw) ----
extern "C" __global__ void __launch_bounds__(256) aq6k8v9(
    const unsigned char* __restrict__ wq, const unsigned char* __restrict__ wk, const unsigned char* __restrict__ wv4,
    const float* __restrict__ gridf, const __half* __restrict__ xh4,
    __half* __restrict__ qrow4, __half* __restrict__ krow4, __half* __restrict__ vrow4)
{
  ...
  if (warp < 12288) {
    ...
    RED9(a0,a1,a2,a3,a4,a5,a6,a7,a8)
    if (lane == 0) { qrow4[0*12288+warp] = (__half)a0; qrow4[1*12288+warp] = (__half)a1; qrow4[2*12288+warp] = (__half)a2; qrow4[3*12288+warp] = (__half)a3; qrow4[4*12288+warp] = (__half)a4; qrow4[5*12288+warp] = (__half)a5; qrow4[6*12288+warp] = (__half)a6; qrow4[7*12288+warp] = (__half)a7; qrow4[8*12288+warp] = (__half)a8; }
  } else if (warp < 13312) {
    const int r = warp - 12288;
