...
// W2E KV8: int8-KV append KPRE. IDENTICAL to the fp16 KPRE (spk_pre{1,3}_*)
// for the q-path (qw output bit-identical: q-norm + partial-rope + 0.0625
// fold untouched) and the k-norm/rope math; ONLY the KV store quantizes:
// the fp16 value the fp16 path would store (K: __float2half(ko); V: vrow
// verbatim) is quantized per-(row, 32-channel) group -> scale = fp16(max|v|/127),
// int8 = round(v / scale) clipped to [-127,127]. Thread layout: d = threadIdx.x
// (one channel/thread, 256 threads) so each WARP is exactly one 32-channel
// group -> warp butterfly max, no cross-warp reduction needed.
// sc layout: [2][4][CTXK][8] fp16 (K groups 0-3, V groups 4-7), same slab
// order as kv. LAWS: full-mask shuffles in convergent code (h is CTA-uniform),
// sequential t loop, no local arrays. -DKNAME -DCTXK -DROWS (3|1)
#include <cuda_fp16.h>
  ...
  for (int t = 0; t < ROWS; ++t) {
    ...
    }
    {
      ...
      const float ko = (d < 32) ? kvv*cs - sm[256 + d + 32]*sn
                     ...
                     : kvv;
      if ((h % 6) == 0) {
        // ---- int8 store: warp == one 32-channel scale group ----
        ...
        const float vf = __half2float(vrow[(ROWS==3 ? t*1024 : 0) + kvh*256 + d]);
        ...
        Kc[(size_t)(pos+t)*256 + d] = (unsigned char)(iq + 128);   // biased uint8
