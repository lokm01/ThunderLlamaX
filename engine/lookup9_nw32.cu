...
// R7a K=8 LOOKUP9 drafter: deep-K (K=8) n-gram proposal kernel.
// Same contract as lookup8_nw32 (R5d) with the DEEP-K SCAN-RANGE LAW at K=8:
// iend = pos - 8 - K = pos - 17 (all 8 proposal tokens hist[i+8..i+15] are
// WRITTEN ids <= hist[pos-1] = cur). Writes dring0..7 only on hit (l >= 8).
// Single 1024-thread CTA (nw32 name law); thread-strided flat scan.
extern "C" __global__ void __launch_bounds__(1024) lookup9_nw32(
    ...
    int* __restrict__ l_hist, const int* __restrict__ cyc_slot)
{
  ...
  int lkey = 0;
  if (pos >= 17) {   // suffix defined (pos>=8) and scan [0,pos-17] non-empty
  ...
  if (tid == 0) {
    const int key = bkey;
    const int l = key >> 20;
    l_hist[cyc_slot[0]] = (pos < 17) ? (1000000 + pos) : (l + 1);   // 1..8 miss; 9 hit
    if (l >= 8) {
