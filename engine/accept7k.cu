...
// R5 K=6 accept7k: the DEEP (K=6) accept — m-ladder to 6 over dring0..5.
// 18-word emit record: {pos_new, m, tok0..6, stop, cyc, hit, rsv...}; DecodeSession
// reads layout-aware (18 when the K=6 deep set ran this cycle). emit[10] = l_hist[cyc]
// (9 = 8-gram hit) = the selection flag. h_seed = xA[m*DIM] (m <= 6 — T=7 rows).
// dhd_seed law unchanged (draft numerics carry no exactness contract).
#include <cuda_fp16.h>
#define DIM 5120
extern "C" __global__ void __launch_bounds__(256) accept7k(
    const int* __restrict__ amds, const int* __restrict__ dring0, const int* __restrict__ dring1,
    const int* __restrict__ dring2, const int* __restrict__ dring3, const int* __restrict__ dring4, const int* __restrict__ dring5,
    ...
    float* __restrict__ dhd_seed, int* __restrict__ emit, const int* __restrict__ l_hist)
{
  ...
  if (threadIdx.x == 0) {
    ...
    emit[6] = amds[4]; emit[7] = amds[5]; emit[8] = amds[6];
    emit[9] = 0;                    // stop_flag
    emit[10] = cyc;
    emit[11] = l_hist[cyc];         // R5: hit flag (9 = 8-gram hit)
    cyc_slot[0] = cyc + 1;
    ms = m;
  }
  __syncthreads();
