...
#include <cuda_fp16.h>
// W2F L4: k2s3v = grid-split k2s3 (48 -> 384 CTAs, block x 8 substripes of 16
// v-rows; each warp does 2 rows instead of 16). Prologue (conv+qk-norm+q/k/v
// writes) replicated identically in every substripe (idempotent same-value
// writes, same arithmetic); conv-window shift done by sub==0 only; the zz RMS
// norm + gated z3 write SPLIT OUT into k2z3 (needs all 128 core rows of the
// head). Outputs BIT-IDENTICAL to k2s3. LAWS: no gridDim reads, full-mask
// shuffles, sequential loops, per-kernel cubin.
...
extern "C" __global__ void __launch_bounds__(256) k2s3v(
    ...
    float* __restrict__ core, const float* __restrict__ snw, __half* __restrict__ z3)
{
  ...
  for (int t = 0; t < 3; ++t) {
    ...
    for (int vv2 = 0; vv2 < 2; ++vv2) {
      const int v_idx = sub*16 + warp*2 + vv2;
