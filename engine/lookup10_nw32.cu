...
// R8 K=9 LOOKUP10 drafter: deep-K (K=9) n-gram proposal kernel.
// Same contract as lookup9_nw32 (R7a) with the DEEP-K SCAN-RANGE LAW at K=9:
// iend = pos - 8 - (K+1) = pos - 18 (mirrors lookup9's conservative-by-one
// reach; all 9 proposal tokens hist[i+8..i+16] are WRITTEN ids <= hist[pos-2]).
// Writes dring0..8 only on hit (l >= 8 = LMIN full window).
// Single 1024-thread CTA (nw32 name law); thread-strided flat scan.
extern "C" __global__ void __launch_bounds__(1024) lookup10_nw32(
    ...
    int* __restrict__ dring2, int* __restrict__ dring3, int* __restrict__ dring4,
    int* __restrict__ dring5, int* __restrict__ dring6, int* __restrict__ dring7,
    int* __restrict__ dring8,
    int* __restrict__ l_hist, const int* __restrict__ cyc_slot)
{
  ...
  int lkey = 0;
  if (pos >= 18) {   // suffix defined (pos>=8) and scan [0,pos-18] non-empty
  ...
  if (tid == 0) {
    const int key = bkey;
    const int l = key >> 20;
    l_hist[cyc_slot[0]] = (pos < 18) ? (1000000 + pos) : (l + 1);   // 1..8 miss; 9 hit
    if (l >= 8) {
      ...
      dring6[0] = tok_hist[i + 14];
      dring7[0] = tok_hist[i + 15];
      dring8[0] = tok_hist[i + 16];
    }
  }
}
