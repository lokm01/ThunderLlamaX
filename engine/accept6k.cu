...
// R5 K=5 accept6k: the DEEP (K=5) accept — m-ladder to 5 over dring0..4.
// 17-word emit record: {pos_new, m, tok0..5, stop, cyc, hit, rsv...} — one word
// LONGER than the K=4 16-word layout (tok5); DecodeSession reads layout-aware
// (17 when the K=5 deep set ran this cycle, else 16). emit[10] = l_hist[cyc]
// (9 = 8-gram hit) = the selection flag. h_seed = xA[m*DIM] (m <= 5 — T=6 rows).
// dhd_seed law unchanged (draft numerics carry no exactness contract).
#include <cuda_fp16.h>
#define DIM 5120
extern "C" __global__ void __launch_bounds__(256) accept6k(
    ...
    float* __restrict__ dhd_seed, int* __restrict__ emit, const int* __restrict__ l_hist)
{
  ...
  if (threadIdx.x == 0) {
    ...
    emit[6] = amds[4]; emit[7] = amds[5];
    emit[8] = 0;                    // stop_flag
    emit[9] = cyc;
    emit[10] = l_hist[cyc];         // R5: hit flag (9 = 8-gram hit)
    cyc_slot[0] = cyc + 1;
    ms = m;
  }
  __syncthreads();
