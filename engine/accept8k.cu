...
// R5d K=7 accept8k: the DEEP (K=7) accept — m-ladder to 7 over dring0..6.
// 19-word emit record: {pos_new, m, tok0..7, stop, cyc, hit, rsv...}; DecodeSession
// reads layout-aware (19 when the K=7 deep set ran this cycle). emit[12] = l_hist[cyc]
// (9 = 8-gram hit) = the selection flag. h_seed = xA[m*DIM] (m <= 7 — T=8 rows).
// dhd_seed law unchanged (draft numerics carry no exactness contract).
#include <cuda_fp16.h>
#define DIM 5120
extern "C" __global__ void __launch_bounds__(256) accept8k(
    ...
    const int* __restrict__ dring5, const int* __restrict__ dring6,
    ...
    float* __restrict__ dhd_seed, int* __restrict__ emit, const int* __restrict__ l_hist)
{
  ...
  if (threadIdx.x == 0) {
    ...
    emit[10] = 0;                   // stop_flag
    emit[11] = cyc;
    emit[12] = l_hist[cyc];         // R5d: hit flag (9 = 8-gram hit)
    cyc_slot[0] = cyc + 1;
    ms = m;
  }
  __syncthreads();
