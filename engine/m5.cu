...
// ---- h_embed4 ----
extern "C" __global__ void __launch_bounds__(256) h_embed5(
    const unsigned char* __restrict__ emb, const float* __restrict__ grid512,
    const int* __restrict__ s0, const int* __restrict__ s1, const int* __restrict__ s2, const int* __restrict__ s3, const int* __restrict__ s4,
    float* __restrict__ x4)
{
  const int tid = threadIdx.x;
  const int toks[5] = { s0[0], s1[0], s2[0], s3[0], s4[0] };
  for (int t = 0; t < 5; ++t) {
...
// ---- k0n4 ----
extern "C" __global__ void __launch_bounds__(256) k0n5(
...
// ---- q5g8v4 ----
extern "C" __global__ void __launch_bounds__(256) q5g8v5(
    ...
    __half* __restrict__ qkv4, __half* __restrict__ gate4)
{
  ...
  if (warp < 10240) {
    ...
    _Pragma("unroll 5") for (int b = 0; b < 20; ++b) {
      ...
      LDH2(xv0, xh4, DIM, 0, koff) LDH2(xv1, xh4, DIM, 1, koff) LDH2(xv2, xh4, DIM, 2, koff) LDH2(xv3, xh4, DIM, 3, koff) LDH2(xv4, xh4, DIM, 4, koff)
      float wv[8];
    ...
    RED5(a0,a1,a2,a3,a4)
    if (lane == 0) { qkv4[0*10240+warp] = (__half)a0; qkv4[1*10240+warp] = (__half)a1; qkv4[2*10240+warp] = (__half)a2; qkv4[3*10240+warp] = (__half)a3; qkv4[4*10240+warp] = (__half)a4; }
  } else {
    ...
    RED5(a0,a1,a2,a3,a4)
    if (lane == 0) { gate4[0*6144+r] = (__half)a0; gate4[1*6144+r] = (__half)a1; gate4[2*6144+r] = (__half)a2; gate4[3*6144+r] = (__half)a3; gate4[4*6144+r] = (__half)a4; }
  }
}

// ---- k2s4: conv+silu+L2+delta-scan 4 steps, per-step slots 0..3 + live 4 ----
extern "C" __global__ void __launch_bounds__(256) k2s5(
    ...
    float* __restrict__ core, const float* __restrict__ snw, __half* __restrict__ z4)
{
  ...
  for (int t = 0; t < 5; ++t) {
    ...
    float* rec_out = rec_b + (size_t)t*(NVH*128*128) + (size_t)h*128*128;
    #pragma unroll
...
// ---- op38nw32_4 ----
extern "C" __global__ void __launch_bounds__(1024) op38nw32_5(
    ...
    const __half* __restrict__ z4, __half* __restrict__ attn_out4)
{
  ...
  for (int b = 0; b < 24; ++b) {
    ...
    const float4 g1 = *((const float4*)(gridf + ((q >> 8) << 2)));
    const int koff = (b << 8) + (lane << 3);
    LDH2(xv0, z4, 6144, 0, koff) LDH2(xv1, z4, 6144, 1, koff) LDH2(xv2, z4, 6144, 2, koff) LDH2(xv3, z4, 6144, 3, koff) LDH2(xv4, z4, 6144, 4, koff)
    const float sg0 = ((sidx>>0)&1) ? -1.f : 1.f, sg1 = ((sidx>>1)&1) ? -1.f : 1.f;
    const float sg2 = ((sidx>>2)&1) ? -1.f : 1.f, sg3 = ((sidx>>3)&1) ? -1.f : 1.f;
    const float sg4 = ((sidx>>4)&1) ? -1.f : 1.f, sg5 = ((sidx>>5)&1) ? -1.f : 1.f;
    const float sg6 = ((sidx>>6)&1) ? -1.f : 1.f, sg7 = spar ? -1.f : 1.f;
...
// ---- k3aonw32_4 (Q8_0) ----
extern "C" __global__ void __launch_bounds__(1024) k3aonw32_5(
...
// ---- hh4 ----
extern "C" __global__ void __launch_bounds__(256) hh5(
...
// ---- down8nw32_4 ----
extern "C" __global__ void __launch_bounds__(1024) down8nw32_5(
    ...
    const __half* __restrict__ gact4, const float* __restrict__ hh4b, float* __restrict__ y4)
{
  ...
  for (int b = 0; b < 68; ++b) {
    ...
    const float4 g1 = *((const float4*)(gridf + ((q >> 8) << 2)));
    const int koff = (b << 8) + (lane << 3);
    LDH2(xv0, gact4, FFN_N, 0, koff) LDH2(xv1, gact4, FFN_N, 1, koff) LDH2(xv2, gact4, FFN_N, 2, koff) LDH2(xv3, gact4, FFN_N, 3, koff) LDH2(xv4, gact4, FFN_N, 4, koff)
    const float sg0 = ((sidx>>0)&1) ? -1.f : 1.f, sg1 = ((sidx>>1)&1) ? -1.f : 1.f;
    const float sg2 = ((sidx>>2)&1) ? -1.f : 1.f, sg3 = ((sidx>>3)&1) ? -1.f : 1.f;
    const float sg4 = ((sidx>>4)&1) ? -1.f : 1.f, sg5 = ((sidx>>5)&1) ? -1.f : 1.f;
    const float sg6 = ((sidx>>6)&1) ? -1.f : 1.f, sg7 = spar ? -1.f : 1.f;
  ...
  if (lane == 0) {
    ...
    y4[3*DIM+warp] = hh4b[3*DIM+warp] + (float)((__half)a3); y4[4*DIM+warp] = hh4b[4*DIM+warp] + (float)((__half)a4);
  }
}

// ---- aq3k8v4 (q IQ3 packed + k IQ3 packed + v Q4_K raw) ----
extern "C" __global__ void __launch_bounds__(256) aq3k8v5(
  ...
  if (warp < 12288) {
    #define AQI3P4(ROWB, OB, OS, RIDX) { \
      ...
      _Pragma("unroll 5") for (int b = 0; b < 20; ++b) { \
        ...
        ACC5H2(xv0, xv1, xv2, xv3, xv4, wv, a0, a1, a2, a3, a4) } \
      RED5(a0,a1,a2,a3,a4) \
      if (lane == 0) { (OB)[0*(OS)+(RIDX)] = (__half)a0; (OB)[1*(OS)+(RIDX)] = (__half)a1; (OB)[2*(OS)+(RIDX)] = (__half)a2; (OB)[3*(OS)+(RIDX)] = (__half)a3; (OB)[4*(OS)+(RIDX)] = (__half)a4; } }  // R5 fix: the row-4 store was MISSING (gen_m5 slip) -> qrow3 row 4 stale -> wrong amds[4]
    ...
    #define AKI3(ROWB) { \
      ...
      _Pragma("unroll 5") for (int b = 0; b < 20; ++b) { \
        ...
        ACC5H2(xv0, xv1, xv2, xv3, xv4, wv, a0, a1, a2, a3, a4) } \
      RED5(a0,a1,a2,a3,a4) \
      if (lane == 0) { krow4[0*1024+r] = (__half)a0; krow4[1*1024+r] = (__half)a1; krow4[2*1024+r] = (__half)a2; krow4[3*1024+r] = (__half)a3; krow4[4*1024+r] = (__half)a4; } }
    ...
    RED5(a0,a1,a2,a3,a4)
    if (lane == 0) { vrow4[0*1024+r] = (__half)a0; vrow4[1*1024+r] = (__half)a1; vrow4[2*1024+r] = (__half)a2; vrow4[3*1024+r] = (__half)a3; vrow4[4*1024+r] = (__half)a4; }
  }
}

// ---- head8v4 ----
extern "C" __global__ void __launch_bounds__(256) head8v5(
    const unsigned char* __restrict__ wq5, const __half* __restrict__ xh4, __half* __restrict__ logits4)
{
  ...
  RED5(a0,a1,a2,a3,a4)
  if (lane == 0) { logits4[0*VOCAB+warp] = (__half)a0; logits4[1*VOCAB+warp] = (__half)a1; logits4[2*VOCAB+warp] = (__half)a2; logits4[3*VOCAB+warp] = (__half)a3; logits4[4*VOCAB+warp] = (__half)a4; }
}

// ---- aq6k8v4 (q Q6_K packed + k IQ3 packed + v Q4_K raw) ----
extern "C" __global__ void __launch_bounds__(256) aq6k8v5(
    const unsigned char* __restrict__ wq, const unsigned char* __restrict__ wk, const unsigned char* __restrict__ wv4,
    const float* __restrict__ gridf, const __half* __restrict__ xh4,
    __half* __restrict__ qrow4, __half* __restrict__ krow4, __half* __restrict__ vrow4)
{
  ...
  if (warp < 12288) {
    ...
    RED5(a0,a1,a2,a3,a4)
    if (lane == 0) { qrow4[0*12288+warp] = (__half)a0; qrow4[1*12288+warp] = (__half)a1; qrow4[2*12288+warp] = (__half)a2; qrow4[3*12288+warp] = (__half)a3; qrow4[4*12288+warp] = (__half)a4; }
  } else if (warp < 13312) {
    const int r = warp - 12288;
