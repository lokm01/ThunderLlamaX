...
# ================= P13 variant ladder v2 =================
pr7 = prog("pfg3_ffn_r7_m32_nw8k128")
def V(tag, xb, ob):
  pr7(P.d["r_fg"], P.d["r_fu"], P.d["gridf"], xb, ob, global_size=(272, 1, 1), local_size=LS)
  dev.synchronize(); print(f"[V] {tag} CLEAN", flush=True)
def va(nm): return hex(P.d[nm].va_addr)
P.up("x64p", (np.random.default_rng(3).standard_normal((64, 5120)) * 0.8).astype(np.float16).reshape(-1))
P.poison("o64p", 64 * 17408, np.float16, 7.7)
dev.synchronize()
for nm in ("xffn","mineffn","x64p","o64p","r_fg","r_fu","gridf"):
  print(f"[va] {nm} {va(nm)}", flush=True)
V("V2a-p7bx-freshO", P.d["xffn"], P.d["o64p"])
V("V2b-freshX-p7bo", P.d["x64p"], P.d["mineffn"])
V("V2c-freshX-freshO", P.d["x64p"], P.d["o64p"])
print("[V] ALL CLEAN", flush=True)
# ================= P13 kernel B: persistent FFN probe =================
NBLK = 8
WBLK = 272 * 8 * 40 * 32 * 16
WB = 2 * 17408 * 1960
R7F = 512.0 / 392.0
fgc = np.concatenate([np.load(f"{P7}/fg{b}.npy") for b in range(NBLK)])
fuc = np.concatenate([np.load(f"{P7}/fu{b}.npy") for b in range(NBLK)])
assert fgc.size == NBLK * WBLK and fuc.size == NBLK * WBLK
P.up("w1c", fgc); P.up("w2c", fuc); del fgc, fuc
P.up("x8", (np.random.default_rng(11).standard_normal((NBLK, 32, 5120)) * 0.8).astype(np.float16).reshape(-1))
P.poison("oref8", NBLK * 32 * 17408, np.float16, 7.7)
P.poison("omine8", NBLK * 32 * 17408, np.float16, 7.7)
dev.synchronize()
print("[p13b] concat weights + bufs up", flush=True)
# shipped-ref over the 8 blocks (offset launches — the exact ship-path pattern)
for b in range(NBLK):
  w1b = P.d["w1c"].offset(offset=b * WBLK, size=WBLK)
  w2b = P.d["w2c"].offset(offset=b * WBLK, size=WBLK)
  xb = P.d["x8"].offset(offset=b * 32 * 5120 * 2, size=32 * 5120 * 2)
  ob = P.d["oref8"].offset(offset=b * 32 * 17408 * 2, size=32 * 17408 * 2)
  pr7(w1b, w2b, P.d["gridf"], xb, ob, global_size=(272, 1, 1), local_size=LS)
dev.synchronize()
print("[p13b] shipped-ref 8 blocks CLEAN", flush=True)
ref8 = P.down("oref8", (NBLK, 32, 17408), np.float16)
import time as _t
def bench(fn, n=10):
  fn(); dev.synchronize()
  best = 1e9
  for _ in range(n):
    t0 = _t.perf_counter(); fn(); dev.synchronize()
    best = min(best, _t.perf_counter() - t0)
  return best
def ship_one():
  for b in range(NBLK):
    ...
    w2b = P.d["w2c"].offset(offset=b * WBLK, size=WBLK)
    xb = P.d["x8"].offset(offset=b * 32 * 5120 * 2, size=32 * 5120 * 2)
    ob = P.d["omine8"].offset(offset=b * 32 * 17408 * 2, size=32 * 17408 * 2)
    pr7(w1b, w2b, P.d["gridf"], xb, ob, global_size=(272, 1, 1), local_size=LS)
t_ship = bench(ship_one) / NBLK
print(f"[p13b] SHIP-CONTROL {t_ship*1e6:7.1f} us/blk | amort {2*WB/t_ship/1e9:6.1f} GB/s", flush=True)
for kn in ("pf13ffn_w2_nw8", "pf13ffn_w4_nw8", "pf13ffn_w8_nw8"):
  prp = prog(kn)
  P.poison("omine8", NBLK * 32 * 17408, np.float16, 7.7)
  dev.synchronize()
  prp(P.d["w1c"], P.d["w2c"], P.d["gridf"], P.d["x8"], P.d["omine8"], global_size=(82, 1, 1), local_size=LS)
  dev.synchronize()
  mine8 = P.down("omine8", (NBLK, 32, 17408), np.float16)
  nz = int((mine8 != ref8).sum())
  print(f"[p13b-gate] {kn}: {'BIT-IDENTICAL' if nz == 0 else f'DIFF nz={nz}'}", flush=True)
  if nz: continue
  def one():
    prp(P.d["w1c"], P.d["w2c"], P.d["gridf"], P.d["x8"], P.d["omine8"], global_size=(82, 1, 1), local_size=LS)
  t = bench(one) / NBLK
  print(f"[p13b-bench] {kn}: {t*1e6:7.1f} us/blk | x{t_ship/t:4.2f} vs ship | "
        f"w {WB/t/1e9:6.1f} | amort {2*WB/t/1e9:6.1f} | dram {R7F*WB/t/1e9:6.1f} GB/s | "
        f"gate {'PASS' if 2*WB/t/1e9 >= 350 else 'FAIL'}", flush=True)
