...
// R8 K=10 LOOKUP11 drafter: deep-K (K=10) n-gram proposal kernel.
// Same contract as lookup10_nw32 (R8) with the DEEP-K SCAN-RANGE LAW at K=10:
// iend = pos - 8 - (K+1) = pos - 19. Writes dring0..9 only on hit (l >= 8).
// Single 1024-thread CTA (nw32 name law); thread-strided flat scan.
extern "C" __global__ void __launch_bounds__(1024) lookup11_nw32(
    ...
    int* __restrict__ dring8, int* __restrict__ dring9,
    int* __restrict__ l_hist, const int* __restrict__ cyc_slot)
{
  ...
  int lkey = 0;
  if (pos >= 19) {   // suffix defined (pos>=8) and scan [0,pos-19] non-empty
  ...
  if (tid == 0) {
    const int key = bkey;
    const int l = key >> 20;
    l_hist[cyc_slot[0]] = (pos < 19) ? (1000000 + pos) : (l + 1);   // 1..8 miss; 9 hit
    if (l >= 8) {
