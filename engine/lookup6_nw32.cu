...
// R5 K=5 LOOKUP6 drafter: deep-K (K=5) n-gram proposal kernel.
// Same contract as lookup5_nw32 (R4) with the DEEP-K SCAN-RANGE LAW at K=5:
// iend = pos - 8 - K = pos - 13 (all 5 proposal tokens hist[i+8..i+12] are
// WRITTEN ids <= hist[pos-1] = cur). Writes dring0..4 only on hit (l >= 8);
...
extern "C" __global__ void __launch_bounds__(1024) lookup6_nw32(
    ...
    int* __restrict__ l_hist, const int* __restrict__ cyc_slot)
{
  ...
  int lkey = 0;
  if (pos >= 13) {   // suffix defined (pos>=8) and scan [0,pos-13] non-empty
  ...
  if (tid == 0) {
    const int key = bkey;
    const int l = key >> 20;
    l_hist[cyc_slot[0]] = (pos < 13) ? (1000000 + pos) : (l + 1);   // 1..8 miss; 9 hit
    if (l >= 8) {
