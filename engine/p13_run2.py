...
# ================= P13 kernel B final: no-offset protocol =================
NBLK = 8
WBLK = 272 * 8 * 40 * 32 * 16
WB = 2 * 17408 * 1960
R7F = 512.0 / 392.0
...
for b in range(NBLK):
  P.up(f"fgx{b}", np.load(f"{P7}/fg{b}.npy"))
  P.up(f"fux{b}", np.load(f"{P7}/fu{b}.npy"))
...
dev.synchronize()
print("[p13b] all weights up (concat + per-block direct)", flush=True)
# REF: per-block DIRECT bases; x/out offset views on small parents (p7b-proven)
for b in range(NBLK):
  xb = P.d["x8"].offset(offset=b * 32 * 5120 * 2, size=32 * 5120 * 2)
  ob = P.d["oref8"].offset(offset=b * 32 * 17408 * 2, size=32 * 17408 * 2)
  pr7(P.d[f"fgx{b}"], P.d[f"fux{b}"], P.d["gridf"], xb, ob, global_size=(272, 1, 1), local_size=LS)
dev.synchronize()
print("[p13b] shipped-ref 8 blocks (direct bases) CLEAN", flush=True)
...
for kn in ("pf13ffn_w2_nw8", "pf13ffn_w4_nw8", "pf13ffn_w8_nw8"):
  ...
  print(f"[p13b-gate] {kn}: {'BIT-IDENTICAL' if nz == 0 else f'DIFF nz={nz}/{mine8.size}'}", flush=True)
  if nz: continue
  def one():
    prp(P.d["w1c"], P.d["w2c"], P.d["gridf"], P.d["x8"], P.d["omine8"], global_size=(82, 1, 1), local_size=LS)
  t = bench(one) / NBLK
