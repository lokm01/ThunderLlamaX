...
extern "C" __global__ void __launch_bounds__(256) op38t_3(
...
#define NB8 24
#define RS (98*NB8)
  const unsigned char* rp = wo + (size_t)warp * RS;
  ...
  _Pragma("unroll") for (int g = 0; g < NB8/8; ++g) {
    ...
    _Pragma("unroll") for (int j = 0; j < 8; ++j) {
      ...
      if (b < NB) {
        ...
        LD8(xv0, z3, 6144, 0, koff) LD8(xv1, z3, 6144, 1, koff) LD8(xv2, z3, 6144, 2, koff)
