...
print(f"[dbg6] P={P0} cur0={CUR0}", flush=True)
...
print(f"[dbg6] after 60 T=1 tokens: pos_slot={int(P.down_at('pos_slot', 0, 1)[0])} "
...
sl = sl[:SLICE]
print(f"[dbg6] slice {len(set(sl))} distinct", flush=True)
E.init_draft(sl)
E.fill_draft(ids)
print(f"[dbg6] dring0 after fill: {int(P.down_at('dring0', 0, 1)[0])}", flush=True)
E.build_graphs()
dev.synchronize()
...
for k in range(10):
  ...
  if k < 3 or k == 9:
    print(f"[dbg6] cyc{k}: m={r['m']} amds={P.down_at('amds', 0, 3).tolist()} "
          f"dring=({int(P.down_at('dring0', 0, 1)[0])},{int(P.down_at('dring1', 0, 1)[0])})", flush=True)
print(f"[dbg6] sum(m)={mtot} alpha={mtot/20:.3f} -> {'WORKS' if mtot >= 12 else 'BROKEN'}", flush=True)
