...
// ---- h_embed4 ----
extern "C" __global__ void __launch_bounds__(256) h_embed4(
    ...
    float* __restrict__ x4)
{
  ...
  const int toks[4] = { s0[0], s1[0], s2[0], s3[0] };
  for (int t = 0; t < 4; ++t) {
...
// ---- k0n4 ----
extern "C" __global__ void __launch_bounds__(256) k0n4(
...
// ---- k0ab4 ----
extern "C" __global__ void __launch_bounds__(256) k0ab4(
...
// ---- q5g8v4 ----
extern "C" __global__ void __launch_bounds__(256) q5g8v4(
    ...
    __half* __restrict__ qkv4, __half* __restrict__ gate4)
{
  ...
  if (warp < 10240) {
    ...
    RED4(a0,a1,a2,a3)
    if (lane == 0) { qkv4[0*10240+warp] = (__half)a0; qkv4[1*10240+warp] = (__half)a1; qkv4[2*10240+warp] = (__half)a2; qkv4[3*10240+warp] = (__half)a3; }
  } else {
    ...
    RED4(a0,a1,a2,a3)
    if (lane == 0) { gate4[0*6144+r] = (__half)a0; gate4[1*6144+r] = (__half)a1; gate4[2*6144+r] = (__half)a2; gate4[3*6144+r] = (__half)a3; }
  }
}

// ---- k2s4: conv+silu+L2+delta-scan 4 steps, per-step slots 0..3 + live 4 ----
extern "C" __global__ void __launch_bounds__(256) k2s4(
...
// ---- k3aonw32_4 (Q8_0) ----
extern "C" __global__ void __launch_bounds__(1024) k3aonw32_4(
...
// ---- ao8nw32_4 (IQ3_S raw) ----
extern "C" __global__ void __launch_bounds__(1024) ao8nw32_4(
    ...
    const __half* __restrict__ ao_in4, __half* __restrict__ attn_out4)
{
  ...
  for (int b = 0; b < 24; ++b) {
    ...
    const int koff = (b << 8) + (lane << 3);
    LDH2(xv0, ao_in4, 6144, 0, koff) LDH2(xv1, ao_in4, 6144, 1, koff) LDH2(xv2, ao_in4, 6144, 2, koff) LDH2(xv3, ao_in4, 6144, 3, koff)
...
// ---- hh4 ----
extern "C" __global__ void __launch_bounds__(256) hh4(
...
// ---- ffn8v4 ----
extern "C" __global__ void __launch_bounds__(256) ffn8v4(
    ...
    const float* __restrict__ gridf, const __half* __restrict__ hhx4, __half* __restrict__ gact4)
{
  ...
  float ag0=0.f,ag1=0.f,ag2=0.f,ag3=0.f, au0=0.f,au1=0.f,au2=0.f,au3=0.f;
  #pragma unroll 5
  for (int b = 0; b < 20; ++b) {
    const int koff = (b << 8) + (lane << 3);
    LDH2(xg0, hhx4, DIM, 0, koff) LDH2(xg1, hhx4, DIM, 1, koff) LDH2(xg2, hhx4, DIM, 2, koff) LDH2(xg3, hhx4, DIM, 3, koff)
    #define IQ3V4(QP, SP, DP, A0, A1, A2, A3) { \
      ...
      const float sg6 = ((sidx>>6)&1) ? -1.f : 1.f, sg7 = spar ? -1.f : 1.f; \
      float wv[8] = { db*g0.x*sg0, db*g0.y*sg1, db*g0.z*sg2, db*g0.w*sg3, db*g1.x*sg4, db*g1.y*sg5, db*g1.z*sg6, db*g1.w*sg7 }; \
      ACC4H2(xg0, xg1, xg2, xg3, wv, A0, A1, A2, A3) }
    IQ3V4(qg, sg, dg, ag0, ag1, ag2, ag3)
    IQ3V4(qu, su, du, au0, au1, au2, au3)
    #undef IQ3V4
  }
  RED4(ag0,ag1,ag2,ag3)
  RED4(au0,au1,au2,au3)
  if (lane == 0) {
    ...
    gact4[3*FFN_N + warp] = __hmul(hsilu_h4((__half)ag3), (__half)au3);
  }
}

// ---- down8nw32_4 ----
extern "C" __global__ void __launch_bounds__(1024) down8nw32_4(
    ...
    const __half* __restrict__ gact4, const float* __restrict__ hh4b, float* __restrict__ y4)
{
  ...
  for (int b = 0; b < 68; ++b) {
    ...
    const float4 g1 = *((const float4*)(gridf + ((q >> 8) << 2)));
    const int koff = (b << 8) + (lane << 3);
    LDH2(xv0, gact4, FFN_N, 0, koff) LDH2(xv1, gact4, FFN_N, 1, koff) LDH2(xv2, gact4, FFN_N, 2, koff) LDH2(xv3, gact4, FFN_N, 3, koff)
    const float sg0 = ((sidx>>0)&1) ? -1.f : 1.f, sg1 = ((sidx>>1)&1) ? -1.f : 1.f;
    const float sg2 = ((sidx>>2)&1) ? -1.f : 1.f, sg3 = ((sidx>>3)&1) ? -1.f : 1.f;
    const float sg4 = ((sidx>>4)&1) ? -1.f : 1.f, sg5 = ((sidx>>5)&1) ? -1.f : 1.f;
    const float sg6 = ((sidx>>6)&1) ? -1.f : 1.f, sg7 = spar ? -1.f : 1.f;
...
// ---- head8v4 ----
extern "C" __global__ void __launch_bounds__(256) head8v4(
    const unsigned char* __restrict__ wq5, const __half* __restrict__ xh4, __half* __restrict__ logits4)
{
  ...
  RED4(a0,a1,a2,a3)
  if (lane == 0) { logits4[0*VOCAB+warp] = (__half)a0; logits4[1*VOCAB+warp] = (__half)a1; logits4[2*VOCAB+warp] = (__half)a2; logits4[3*VOCAB+warp] = (__half)a3; }
}

// ---- aq6k8v4 (q Q6_K packed + k IQ3 packed + v Q4_K raw) ----
extern "C" __global__ void __launch_bounds__(256) aq6k8v4(
    const unsigned char* __restrict__ wq, const unsigned char* __restrict__ wk, const unsigned char* __restrict__ wv4,
    const float* __restrict__ gridf, const __half* __restrict__ xh4,
    __half* __restrict__ qrow4, __half* __restrict__ krow4, __half* __restrict__ vrow4)
{
  ...
  if (warp < 12288) {
    ...
    RED4(a0,a1,a2,a3)
    if (lane == 0) { qrow4[0*12288+warp] = (__half)a0; qrow4[1*12288+warp] = (__half)a1; qrow4[2*12288+warp] = (__half)a2; qrow4[3*12288+warp] = (__half)a3; }
  } else if (warp < 13312) {
    const int r = warp - 12288;
