...
extern "C" __global__ void __launch_bounds__(256) ffn8r7(
    ...
    const float* __restrict__ gridf, const __half* __restrict__ hhx, __half* __restrict__ gact)
{
  ...
  if (lane == 0) gact[warp] = __hmul(hsilu_hr((__half)ag), (__half)au);
}

// ---- down8r7: down GEMV IQ3 r7 [5120, 17408] + residual, M=1 (port of down8) ----
extern "C" __global__ void __launch_bounds__(256) down8r7(
...
// ---- ffn8v3r7: gate+up IQ3 r7 GEMVs + silu-mul, M=3 (port of ffn8v_3) ----
extern "C" __global__ void __launch_bounds__(256) ffn8v3r7(
    const unsigned char* __restrict__ wg, const unsigned char* __restrict__ wu,
    const float* __restrict__ gridf, const __half* __restrict__ hhx3, __half* __restrict__ gact3)
{
  ...
  RED3(au0,au1,au2)
  if (lane == 0) {
    gact3[0*FFN_N + warp] = __hmul(hsilu_hr((__half)ag0), (__half)au0);
    gact3[1*FFN_N + warp] = __hmul(hsilu_hr((__half)ag1), (__half)au1);
    gact3[2*FFN_N + warp] = __hmul(hsilu_hr((__half)ag2), (__half)au2);
  }
}

// ---- down8nw32v3r7: down GEMV IQ3 r7 + residual, M=3 fat-CTA (port of down8nw32_3) ----
extern "C" __global__ void __launch_bounds__(1024) down8nw32v3r7(
    const unsigned char* __restrict__ wd, const float* __restrict__ gridf,
...
// ---- ffn8v8r7: gate+up IQ3 r7 GEMVs + silu-mul, M=8 (port of ffn8v8) ----
extern "C" __global__ void __launch_bounds__(256) ffn8v8r7(
    const unsigned char* __restrict__ wg, const unsigned char* __restrict__ wu,
    const float* __restrict__ gridf, const __half* __restrict__ hhx4, __half* __restrict__ gact4)
{
  ...
  for (int b = 0; b < 20; ++b) {
    ...
    #define R7V8(W, A0, A1, A2, A3, A4, A5, A6, A7) { \
      R7U(W, 40, b) \
      const float d = __half2float(__ushort_as_half(dr)); \
      const float db = d * (((float)(swr >> 28)) + 0.5f) * 0.5f; \
      const unsigned int sidx = (swr >> (7u * (unsigned int)(lane & 3))) & 0x7Fu; \
    ...
    R7V8(wg, ag0, ag1, ag2, ag3, ag4, ag5, ag6, ag7)
    R7V8(wu, au0, au1, au2, au3, au4, au5, au6, au7)
    #undef R7V8
  }
...
extern "C" __global__ void __launch_bounds__(1024) down8nw32v8r7(
    const unsigned char* __restrict__ wd, const float* __restrict__ gridf,
    const __half* __restrict__ gact4, const float* __restrict__ hh4b, float* __restrict__ y4)
{
  ...
  if (lane == 0) {
    ...
    y4[3*DIM+warp] = hh4b[3*DIM+warp] + (float)((__half)a3);
    y4[4*DIM+warp] = hh4b[4*DIM+warp] + (float)((__half)a4);
    y4[5*DIM+warp] = hh4b[5*DIM+warp] + (float)((__half)a5);
    y4[6*DIM+warp] = hh4b[6*DIM+warp] + (float)((__half)a6);
    y4[7*DIM+warp] = hh4b[7*DIM+warp] + (float)((__half)a7);
  }
}

// ---- ffn8v9r7: gate+up IQ3 r7 GEMVs + silu-mul, M=9 (R7a K=8, port of ffn8v9) ----
extern "C" __global__ void __launch_bounds__(256) ffn8v9r7(
    const unsigned char* __restrict__ wg, const unsigned char* __restrict__ wu,
    const float* __restrict__ gridf, const __half* __restrict__ hhx4, __half* __restrict__ gact4)
{
  const int warp = (blockIdx.x << 3) + (threadIdx.x >> 5);
  ...
  for (int b = 0; b < 20; ++b) {
    ...
    #define R7V9(W, A0, A1, A2, A3, A4, A5, A6, A7, A8) { \
      R7U(W, 40, b) \
      const float d = __half2float(__ushort_as_half(dr)); \
      const float db = d * (((float)(swr >> 28)) + 0.5f) * 0.5f; \
      const unsigned int sidx = (swr >> (7u * (unsigned int)(lane & 3))) & 0x7Fu; \
    ...
    R7V9(wg, ag0, ag1, ag2, ag3, ag4, ag5, ag6, ag7, ag8)
    R7V9(wu, au0, au1, au2, au3, au4, au5, au6, au7, au8)
    #undef R7V9
  }
...
extern "C" __global__ void __launch_bounds__(1024) down8nw32v9r7(
    const unsigned char* __restrict__ wd, const float* __restrict__ gridf,
    const __half* __restrict__ gact4, const float* __restrict__ hh4b, float* __restrict__ y4)
{
  const int warp = (blockIdx.x << 5) + (threadIdx.x >> 5);
...
// ---- ffn8v10r7: gate+up IQ3 r7 GEMVs + silu-mul, M=10 (R8 K=9, port of ffn8v10) ----
extern "C" __global__ void __launch_bounds__(256) ffn8v10r7(
    const unsigned char* __restrict__ wg, const unsigned char* __restrict__ wu,
    const float* __restrict__ gridf, const __half* __restrict__ hhx4, __half* __restrict__ gact4)
{
  const int warp = (blockIdx.x << 3) + (threadIdx.x >> 5);
  ...
  for (int b = 0; b < 20; ++b) {
    ...
    #define R7V10(W, A0, A1, A2, A3, A4, A5, A6, A7, A8, A9) { \
      R7U(W, 40, b) \
      const float d = __half2float(__ushort_as_half(dr)); \
      const float db = d * (((float)(swr >> 28)) + 0.5f) * 0.5f; \
      const unsigned int sidx = (swr >> (7u * (unsigned int)(lane & 3))) & 0x7Fu; \
    ...
    R7V10(wg, ag0, ag1, ag2, ag3, ag4, ag5, ag6, ag7, ag8, ag9)
    R7V10(wu, au0, au1, au2, au3, au4, au5, au6, au7, au8, au9)
    #undef R7V10
  }
...
extern "C" __global__ void __launch_bounds__(1024) down8nw32v10r7(
    const unsigned char* __restrict__ wd, const float* __restrict__ gridf,
    const __half* __restrict__ gact4, const float* __restrict__ hh4b, float* __restrict__ y4)
{
  const int warp = (blockIdx.x << 5) + (threadIdx.x >> 5);
  ...
  #pragma unroll 2  // R8: 64-reg budget at M=10 (fp order per row unchanged)
  for (int b = 0; b < 68; ++b) {
    R7U(wd, 136, b)
    const float d = __half2float(__ushort_as_half(dr));
    const float db = d * (((float)(swr >> 28)) + 0.5f) * 0.5f;
...
// ---- ffn8v11r7: gate+up IQ3 r7 GEMVs + silu-mul, M=11 (R8 K=10, port of ffn8v11) ----
...
extern "C" __global__ void __launch_bounds__(1024) down8nw32v11r7(
    const unsigned char* __restrict__ wd, const float* __restrict__ gridf,
    const __half* __restrict__ gact4, const float* __restrict__ hh4b, float* __restrict__ y4)
{
  const int warp = (blockIdx.x << 5) + (threadIdx.x >> 5);
  ...
  if (lane == 0) {
    ...
    y4[8*DIM+warp] = hh4b[8*DIM+warp] + (float)((__half)a8);
    y4[9*DIM+warp] = hh4b[9*DIM+warp] + (float)((__half)a9);
    y4[10*DIM+warp] = hh4b[10*DIM+warp] + (float)((__half)a10);
  }
}
