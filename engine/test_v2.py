...
P = Bufs()
pr = {}
for n in ["ffn8_3","down8_3","ffn8v_3","down8v_3","ffn8w32_3","down8w32_3"]:
...
def run_old_new(kold, knew, args_old, args_new, outnm, out_bytes, dt, cmp_fn=None):
  ...
  _g = (544,1,1) if "w32" in kold else (2176,1,1) if "ffn" in kold else (640,1,1)
  _l = (1024,1,1) if "w32" in kold else LS
  pr[kold](*args_old, global_size=_g, local_size=_l, wait=True)
  old = P.down(outnm, (out_bytes//np.dtype(dt).itemsize,), dt)
  # new
  P.poison(outnm, out_bytes, dt, 7.7 if dt==np.float16 else 7.7e31)
  _g = (544,1,1) if "w32" in knew else (2176,1,1) if "ffn" in knew else (640,1,1)
  _l = (1024,1,1) if "w32" in knew else LS
  pr[knew](*args_new, global_size=_g, local_size=_l, wait=True)
...
print("[t] ---- NEW (v2) ----", flush=True)
bench("ffn8v_3", sz["fg8"]+sz["fu8"], lambda w: pr["ffn8v_3"](d["fg8_old"], d["fu8_old"], d["gridf"], d["hhx3"], d["gactA"], global_size=(2176,1,1), local_size=LS, wait=w))
bench("down8v_3", sz["fd8"], lambda w: pr["down8v_3"](d["fd8_old"], d["gridf"], d["gact3_in"], d["hh3f"], d["y3"], global_size=(640,1,1), local_size=LS, wait=w))

bench("ffn8w32", sz["fg8"]+sz["fu8"], lambda w: pr["ffn8w32_3"](d["fg8_old"], d["fu8_old"], d["gridf"], d["hhx3"], d["gactA"], global_size=(544,1,1), local_size=(1024,1,1), wait=w))
bench("down8w32", sz["fd8"], lambda w: pr["down8w32_3"](d["fd8_old"], d["gridf"], d["gact3_in"], d["hh3f"], d["y3"], global_size=(160,1,1), local_size=(1024,1,1), wait=w))
